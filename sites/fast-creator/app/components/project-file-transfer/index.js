/** @jsx h */ /* Note: Set the JSX pragma to the wrapped version of createElement */
import { fastButton } from "@microsoft/fast-components";
import { downChevron, upChevron } from "@microsoft/site-utilities";
import h from "@microsoft/site-utilities/dist/web-components/pragma";
fastButton;
export const ProjectFileTransfer = ({ projectFile, onUpdateProjectFile }) => {
    const downloadElementId = "hiddenProjectFileDownloadElement";
    const uploadElementId = "hiddenProjectFileUploadElement";
    function handleDownloadOnClick(e) {
        e.preventDefault();
        document.getElementById(downloadElementId).click();
    }
    function handleUploadOnChange(e) {
        const reader = new FileReader();
        reader.onload = event => {
            if (event.target && event.target.result && reader.result) {
                onUpdateProjectFile(JSON.parse(reader.result));
            }
        };
        if (e.target.files) {
            reader.readAsText(e.target.files[0]);
        }
    }
    return (
        <div style={{ position: "absolute", bottom: "0", padding: "10px" }}>
            <div style={{ position: "relative", display: "inline-block" }}>
                <input
                    id={uploadElementId}
                    type={"file"}
                    accept={"text/json"}
                    style={{
                        opacity: "0",
                        width: "32px",
                        height: "24px",
                        position: "absolute",
                        left: "0",
                        border: "none",
                        padding: "0",
                        zIndex: 1,
                        cursor: "pointer",
                    }}
                    onChange={handleUploadOnChange}
                />
                <fast-button tabIndex="-1" title="upload a project file">
                    {upChevron()}
                </fast-button>
            </div>
            <div
                style={{
                    position: "relative",
                    display: "inline-block",
                    marginLeft: "4px",
                }}
            >
                <fast-button
                    title="download a project file"
                    events={{
                        click: e => {
                            handleDownloadOnClick(e);
                        },
                    }}
                >
                    {downChevron()}
                </fast-button>
                <a
                    id={downloadElementId}
                    style={{ display: "none" }}
                    href={
                        "data:text/json;charset=utf-8," +
                        encodeURIComponent(JSON.stringify(projectFile))
                    }
                    download={`project-file-${Date.now()}.json`}
                />
            </div>
        </div>
    );
};
export * from "./project-file-transfer.props";
