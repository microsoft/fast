...
`@microsoft/fast-element` was designed for tree-shaking from the beginning, so any feature you don't use when building a component will be removed during build, allowing for highly efficient optimized payloads. See the [Export Sizes](/docs/2.x/resources/export-sizes/) page for detailed per-export bundle sizes.
