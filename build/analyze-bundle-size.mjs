...
async function createBundle(packageName, exportPath = '.') {
  ...
  try {
    ...
    const entryContent = `import { FASTElement } from '${packageName}${exportPath === '.' ? '' : '/' + exportPath}';
...
async function main() {
  ...
  const outputFile = process.argv.find(arg => arg.startsWith('--output-file='))?.split('=')[1] || 'bundle-size.json';
  ...
  if (outputJson) {
    ...
    if (results.bundles.main.exists) {
      ...
      console.log(`  Gzipped: ${(results.bundles.main.gzipped / 1024).toFixed(2)} KB`);
    } else {
    ...
    if (mainResult) {
      console.log(`Main bundle: ${mainResult.formatted.raw} (${mainResult.formatted.gzipped} gzipped)`);
    } else {
