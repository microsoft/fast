...
# Debugging
declare -a args=("$resource_group" "$service" "$az_app_service_plan" "$az_resource_group")
debugService args
...
for application in ${applications[@]}; do
    ...
    title="configuring IPv6 restrictions"
    printStatus "$title"
        {
    ...
    title="creating slot for last-known-good"
    printStatus "$title"
        {
...
exit    
    ...
    echo "configuring hostname ..."
        ...
        az webapp config hostname add --hostname $name.$dns_zone --resource-group $az_resource_group --webapp-name $new_name
    ...
    echo "configuring app insights [$web_app_insights_instrumentation_key] on web app ..." 
        az webapp config appsettings set --resource-group $az_resource_group --name $new_name \
