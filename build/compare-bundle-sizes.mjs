...
async function main() {
  ...
  const prFile = args.find(arg => arg.startsWith('--pr-file='))?.split('=')[1] || 'bundle-size-pr.json';
  const baseFile = args.find(arg => arg.startsWith('--base-file='))?.split('=')[1] || 'bundle-size-base.json';
  const outputFile = args.find(arg => arg.startsWith('--output-file='))?.split('=')[1] || 'bundle-report.md';
  ...
  try {
    ...
    if (prData.bundles.main.exists && baseData.bundles.main.exists) {
      const rawDiff = formatDiff(prData.bundles.main.raw, baseData.bundles.main.raw);
      const gzipDiff = formatDiff(prData.bundles.main.gzipped, baseData.bundles.main.gzipped);

      report += `### ${rawDiff.emoji} Main Bundle (\`@microsoft/fast-element\`)\n\n`;
      report += '| Metric | Current | Previous | Change |\n';
      report += '|--------|---------|----------|--------|\n';
      report += `| Raw Size | ${formatSize(prData.bundles.main.raw)} | ${formatSize(baseData.bundles.main.raw)} | ${rawDiff.formatted} |\n`;
      report += `| Gzipped | ${formatSize(prData.bundles.main.gzipped)} | ${formatSize(baseData.bundles.main.gzipped)} | ${gzipDiff.formatted} |\n\n`;
      ...
      console.log(`   Raw size: ${formatSize(prData.bundles.main.raw)} (${rawDiff.formatted})`);
      console.log(`   Gzipped: ${formatSize(prData.bundles.main.gzipped)} (${gzipDiff.formatted})`);
      ...
      report += '⚠️ **Main bundle not found in base branch** - this might be a new package\n\n';
      console.log('⚠️ Main bundle not found in base branch - this might be a new package');
    }

    report += '---\n';
    report += `*Analysis performed on commit ${prData.commit?.substring(0, 7)} compared to ${baseData.commit?.substring(0, 7)}*\n`;
