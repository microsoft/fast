[@microsoft/fast-components-styles-msft](../README.md) > ["utilities/acrylic"](../modules/_utilities_acrylic_.md)

# External module: "utilities/acrylic"

## Index

### Variables

* [acrylicNoise](_utilities_acrylic_.md#acrylicnoise)

### Functions

* [applyAcrylicMaterial](_utilities_acrylic_.md#applyacrylicmaterial)

---

## Variables

<a id="acrylicnoise"></a>

### `<Const>` acrylicNoise

**● acrylicNoise**: *`string`* = "data:image/png;base64,"

*Defined in [utilities/acrylic.ts:7](https://github.com/Microsoft/fast-dna/blob/164dd3ca/packages/fast-components-styles-msft/src/utilities/acrylic.ts#L7)*

___

## Functions

<a id="applyacrylicmaterial"></a>

###  applyAcrylicMaterial

▸ **applyAcrylicMaterial**(backgroundColor: *`string`*, opacity: *`number`*, fallbackOpacity?: *`number`*, topHighlight?: *`boolean`*): `CSSRules`<[DesignSystem](../interfaces/_design_system_index_.designsystem.md)>

*Defined in [utilities/acrylic.ts:19](https://github.com/Microsoft/fast-dna/blob/164dd3ca/packages/fast-components-styles-msft/src/utilities/acrylic.ts#L19)*

**Parameters:**

| Name | Type | Default value |
| ------ | ------ | ------ |
| backgroundColor | `string` | - |
| opacity | `number` | - |
| `Default value` fallbackOpacity | `number` | 0.9 |
| `Default value` topHighlight | `boolean` | false |

**Returns:** `CSSRules`<[DesignSystem](../interfaces/_design_system_index_.designsystem.md)>

___

