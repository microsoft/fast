...
| Fixture | Description |
|---|---|
| `attribute-map` | Automatic attribute mapping using `attributeMap()` extension. |
| `attribute-map-naming-strategy` | The `attributeNameStrategy` configuration for mapping HTML attribute names to property names via `attributeMap()` options. |
...
| `observer-map` | The `observerMap()` extension for automatic deep reactive observation. |
| `observer-map-config-object` | `observerMap()` and `attributeMap()` extensions without config (equivalent to "all"). |
