...
test.describe("ObserverMap", async () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.goto("/fixtures/extensions/observer-map/");
    ...
    test.describe("simple array index assignment", () => {
        test("should update the first item of a primitive array via index assignment", async ({
            page,
        }) => {
            const items = page.locator(".simple-item");

            await expect(items.first()).toHaveText("foo");

            await page.locator("button:has-text('Update First Item')").click();

            await expect(items.first()).toHaveText("bar");
        });

        test("should update the second item of a primitive array via index assignment", async ({
            page,
        }) => {
            const items = page.locator(".simple-item");

            await expect(items.nth(1)).toHaveText("baz");

            await page.locator("button:has-text('Update Second Item')").click();

            await expect(items.nth(1)).toHaveText("qux");
        });

        test("should leave the unchanged item intact after a single index assignment", async ({
            page,
        }) => {
            const items = page.locator(".simple-item");
