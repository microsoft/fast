...
Every fixture lives in a category subdirectory under `test/fixtures/`:

```
test/fixtures/<category>/<feature>/
...
1. Create a new directory under `test/fixtures/<category>/` with a descriptive kebab-case name (e.g., `bindings/my-feature/`).
...
For fixtures that test deeply nested property reactivity, register the `observerMap` extension factory in the `.define()` call:
