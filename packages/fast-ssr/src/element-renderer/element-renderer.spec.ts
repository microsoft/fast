...
test.describe("FallbackRenderer", () => {
    ...
    test.describe("rendering an element with attributes", () => {
        ...
        test("should render a non-boolean attribute with the values of true or false", () => {
            ...
            const result = consolidate(
                templateRenderer.render(html`
                <${html.partial(name)} aria-expanded="${x => true}"></${html.partial(
                    name
                )}>
                <${html.partial(name)} aria-expanded="${x => false}"></${html.partial(
                    name
                )}>
            `)
            );
        ...
        test("should render an attribute with a string value", () => {
            ...
            const result = consolidate(
                templateRenderer.render(html`
                <${html.partial(name)} attr="${x => "my-str-value"}"></${html.partial(
                    name
                )}>
            `)
            );
...
test.describe("FASTElementRenderer", () => {
    test.describe("should have a 'matchesClass' method", () => {
        test("that returns true when invoked with a class that extends FASTElement ", () => {
            ...
            expect(SyncFASTElementRenderer.matchesClass(MyElement, "", new Map())).toBe(
                true
            );
        });
    ...
    test.describe("rendering stylesheets", () => {
        test(`should render stylesheets as 'style' elements by default`, () => {
            ...
            expect(result).toBe(
                `<styled-element><template shadowrootmode="open" shadowroot="open"><style>:host { display: block; }</style><style>:host { color: red; }</style></template></styled-element>`
            );
        });
        test.skip(`should render stylesheets as 'fast-style' elements when configured`, () => {
            const { templateRenderer } =
                fastSSR(/* Replace w/ configuration when fast-style work is complete{useFASTStyle: true}*/);
            const result = consolidate(
                templateRenderer.render(
                    html`
                        <styled-element></styled-element>
            ...
            expect(result).toBe(
                `<styled-element><template shadowrootmode="open" shadowroot="open"><fast-style style-id="fast-style-0" css=":host { display: block; }"></fast-style><fast-style style-id="fast-style-1" css=":host { color: red; }"></fast-style></template></styled-element>`
            );
        });
    });
        ...
        test("should not evaluate event bindings on the host", () => {
            ...
            FASTElement.define({
                ...
                template: html`
                    <template
                        @click=${() => {
                            throw new Error("Event bindings should not be invoked");
                        }}
                    ></template>
                `,
            });
        ...
        test("should throw error when rendering an attribute with an object value", () => {
            ...
            try {
                ...
                expect(error).toEqual(
                    new Error("Cannot assign attribute 'attr' for element bare-element.")
                );
            }
        });
    });
    ...
    test.describe("emitting events", () => {
        ...
        class TestEventDispatch extends FASTElement {
            ...
            connectedCallback(): void {
                ...
                const e = new CustomEvent<{ data: string }>("test-event", {
                    bubbles: true,
                    detail: { data: "" },
                });
        ...
        test("An element dispatching an event should get it's own handler fired", () => {
            ...
            const result = consolidate(
                templateRenderer.render(
                    html`
                        <test-event-dispatch listen-self></test-event-dispatch>
                    `
                )
            );
            expect(result).toBe(
                `<test-event-dispatch  event-detail="listen-self-success" listen-self><template shadowrootmode="open" shadowroot="open"></template></test-event-dispatch>`
            );
        });
        test("An ancestor with a handler should get it's handler invoked if the event bubbles", () => {
            ...
            const result = consolidate(
                templateRenderer.render(
                    html`
                        <test-event-listener data="bubble-success">
                            <test-event-dispatch></test-event-dispatch>
                        </test-event-listener>
                    `
                )
            );
            expect(result).toBe(
                `<test-event-listener  data="bubble-success"><template shadowrootmode="open" shadowroot="open"></template><test-event-dispatch event-detail="bubble-success"><template shadowrootmode="open" shadowroot="open"></template></test-event-dispatch></test-event-listener>`
            );
        });
        test("Should bubble events to the document", () => {
            ...
            expect(result).toBe(
                `<test-event-dispatch event-detail="document-success"><template shadowrootmode="open" shadowroot="open"></template></test-event-dispatch>`
            );
        });
        test("Should bubble events to the window", () => {
            ...
            expect(result).toBe(
                `<test-event-dispatch event-detail="window-success"><template shadowrootmode="open" shadowroot="open"></template></test-event-dispatch>`
            );
        });
        test("Should not bubble an event that invokes event.stopImmediatePropagation()", () => {
            ...
            const result = consolidate(
                templateRenderer.render(
                    html`
                        <test-event-listener data="stop-immediate-propagation-failure">
                            <test-event-dispatch
                                stop-immediate-prop
                            ></test-event-dispatch>
                        </test-event-listener>
            ...
            expect(result).toBe(
                `<test-event-listener  data="stop-immediate-propagation-failure"><template shadowrootmode="open" shadowroot="open"></template><test-event-dispatch  event-detail="stop-immediate-prop-success" stop-immediate-prop><template shadowrootmode="open" shadowroot="open"></template></test-event-dispatch></test-event-listener>`
            );
        });
        test("Should not bubble an event that invokes event.stopPropagation()", () => {
            ...
            const result = consolidate(
                templateRenderer.render(
                    html`
                        <test-event-listener data="stop-propagation-failure">
                            <test-event-dispatch stop-prop></test-event-dispatch>
                        </test-event-listener>
                    `
                )
            );
            expect(result).toBe(
                `<test-event-listener  data="stop-propagation-failure"><template shadowrootmode="open" shadowroot="open"></template><test-event-dispatch  event-detail="stop-prop-success" stop-prop><template shadowrootmode="open" shadowroot="open"></template></test-event-dispatch></test-event-listener>`
            );
        });
    });
    ...
    test.describe("rendering asynchronously", () => {
        test("should support attribute mutation for the element as a result of PendingTask events", async () => {
            ...
            expect(await consolidateAsync(templateRenderer.render(template))).toBe(
                `<${name} async-resolved><template shadowrootmode="open" shadowroot="open"></template></${name}>`
            );
        });
        ...
        test("should await multiple PendingTaskEvents", async () => {
            ...
            expect(await consolidateAsync(templateRenderer.render(template))).toBe(
                `<${name} async-resolved-one async-resolved-two><template shadowrootmode="open" shadowroot="open"></template></${name}>`
            );
        });
        test("should render template content only displayed after PendingTaskEvent is resolved", async () => {
            ...
            expect(await consolidateAsync(templateRenderer.render(template))).toBe(
                `<${name}><template shadowrootmode="open" shadowroot="open"><h1>Async content success</h1></template></${name}>`
            );
        });
        test("should support nested async rendering scenarios", async () => {
            ...
            const template = html`<${html.partial(name)}><${html.partial(
                name
            )}></${html.partial(name)}></${html.partial(name)}>`;
            const { templateRenderer } = fastSSR({ renderMode: "async" });

            expect(await consolidateAsync(templateRenderer.render(template))).toBe(
                `<${name} async-resolved><template shadowrootmode="open" shadowroot="open"><slot></slot></template><${name} async-resolved><template shadowrootmode="open" shadowroot="open"><slot></slot></template></${name}></${name}>`
            );
        });
        ...
        test("should support asyncronously calling FASTElement.connectedCallback", async () => {
            ...
            expect(await consolidateAsync(templateRenderer.render(template))).toBe(
                `<${name}  value="hello-world"><template shadowrootmode="open" shadowroot="open"><p>attr value: hello-world</p></template></${name}>`
            );
        });
    });
    ...
    test.describe("with hydration markup enabled", () => {
        test("should emit the 'needs-hydration' attribute from `renderAttributes()`", () => {
            ...
            expect(consolidate(renderer.renderAttributes()).split(" ")).toContain(
                "needs-hydration"
            );
        });
    });
