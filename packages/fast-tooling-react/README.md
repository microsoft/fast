...
The `mapDataToCodePreview` function can be used to map data to a Code Preview. The `mapDataToCodePreview` outputs a string formatted as JSX. It uses the interface `CodePreview` which consists of `componentName`, `childNames` (`CodePreviewChildOption[]`)and `data`. `CodePreviewChildOption` are the available children for the component in the Code Preview it consists of `name` and `schema`.
...
import { Heading } from "@microsoft/fast-components-react-msft";
import * as headingSchema from "@microsoft/fast-components-react-msft/dist/heading/heading.schema.json";
import * as badgeSchema from "@microsoft/fast-components-react-msft/dist/badge/badge.schema.json";
import { CodePreview, mapDataToCodePreview } from "@microsoft/fast-tooling-react";
...
const codePreviewData: CodePreview = {
...
function render() {
    ReactDOM.render(
        <div>
            {mapDataToCodePreview(codePreviewData)}
        </div>
        ,
        root
    );
