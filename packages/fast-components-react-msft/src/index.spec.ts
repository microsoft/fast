...
describe("index.ts MSFT component exports", (): void => {
    test("All MSFT components have respective class exports in index.ts", () => {
        ...
        while (curMatch = rxp.exec(data.split("\n"))) {
            found.push(curMatch[1].replace(/\.\//g, "").replace(/-/g, "").toLowerCase());
        }
