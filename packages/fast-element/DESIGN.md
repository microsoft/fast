...
- **Static hydration tracking**: `ElementController` maintains a static `Set` of elements currently being hydrated. `configHydration()` registers lifecycle callbacks (`hydrationStarted`, `elementWillHydrate`, `elementDidHydrate`, `hydrationComplete`). The `hydrationComplete` callback fires via a debounced `setTimeout(0)` after the last element finishes binding, ensuring all async template batches have settled.
