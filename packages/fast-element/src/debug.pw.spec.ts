...
test.describe("The debug module", () => {
    ...
    test.describe("when sending errors", () => {
        test("expect known error message from known error code", () => {
            const key = keyBase++;
        ...
        test("expect unknown error message from unknown error code", () => {
            const error = FAST.error(10);
            expect(error.message).toBe("Unknown Error");
        });

        test("formats error messages with interpolated values", () => {
            const key = keyBase++;
