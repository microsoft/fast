...
function subscribeToComplexObject(obj: ReturnType<typeof createComplexObject>) {
    const names: string[] = [];
    const subscriber = {
        handleChange(subject: any, propertyName: string) {
...
test.describe("The reactive function", () => {
    test("makes all root properties on the object observable", () => {
        const obj = reactive({
            a: 1,
            b: 2,
            c: 3,
    ...
    test("doesn't fail when making the same object observable multiple times", () => {
        const obj = reactive({
            a: 1,
            b: 2,
            c: 3,
    ...
    test("makes properties on array items observable", () => {
    ...
    test("does not deeply convert by default", () => {
    ...
    test("can make deeply observable objects", () => {
    ...
    test("handles circular references", () => {
        ...
        expect(names).toEqual(expect.arrayContaining(["a", "b", "d", "e", "a"]));
