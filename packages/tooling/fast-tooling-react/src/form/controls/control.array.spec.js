...
describe("ArrayControl", () => {
    ...
    test("should add an item to the array if the add button has been clicked", () => {
        ...
        const addButton = rendered.find(`.${managedClasses.arrayControl_addItemButton}`);
    ...
    test("should not show default values if data exists", () => {
        const arrayItem1 = "foo";
        const arrayItem2 = "bar";
        const defaultArrayItem1 = "hello";
        const defaultArrayItem2 = "world";
        const rendered = mount(
            <TestArrayControl
                {...arrayProps}
    ...
    test("should add an invalid class on each array item that is invalid", () => {
        const rendered = mount(
            <TestArrayControl
                {...arrayProps}
                invalidMessage={"foo"}
                validationErrors={[{ dataLocation: "foo.0", invalidMessage: "bar" }]}
                managedClasses={managedClasses}
                dataLocation={"foo"}
                value={["foo", true]}
            />
    ...
    test("should show an invalid message on each invalid array item if the displayInlineValidation prop is passed", () => {
        const invalidItemMessage = "foobarbat";
        const rendered = mount(
            <TestArrayControl
                {...arrayProps}
                invalidMessage={"foo"}
        ...
        const invalidItem = rendered.find(
