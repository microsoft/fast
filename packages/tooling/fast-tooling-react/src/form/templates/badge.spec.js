...
describe("Badge", () => {
    ...
    test("should create an SVG title element if a description has been passed", () => {
        ...
        const rendered = mount(<Badge type={BadgeType.info} description={description} />);
        const title = rendered.find("title");
        expect(title).toHaveLength(1);
        expect(title.text()).toEqual(description);
    });
    test("should add a `className` to an SVG element if it has been passed", () => {
        const className = "bar";
        const rendered = mount(<Badge type={BadgeType.info} className={className} />);
