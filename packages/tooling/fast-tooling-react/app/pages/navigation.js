...
class NavigationTestPage extends React.Component {
    constructor(props) {
        super(props);
        this.handleLinkedDataNavigationOnChange = linkedDataId => {
            return () => {
        ...
        this.handleIncludeType = type => {
            return e => {
                let currentTypes = [].concat(this.state.types || []);
                if (e.target.checked) {
    ...
    renderLinkedDataItems() {
        return Object.entries(children[0]).map(([key, child], index) => {
            return (
                <div key={key}>
