...
import permutate from "../permutator";
...
function createNumberButton(config) {
    const button = document.createElement("button");
    button.innerText = `Get Permutation: ${config.schema.$id}`;
    button.onclick = callNumberPermutationFactory(config);
    content.appendChild(button);
}
function callNumberPermutationFactory(config) {
    return () => {
        permutate(JSON.stringify(config));
    };
}
function createJSONSchemaPreview(config) {
