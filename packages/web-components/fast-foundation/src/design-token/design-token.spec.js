...
import { css, DOM, html, Observable } from "@microsoft/fast-element";
...
function addElement(parent = document.body) {
    const el = document.createElement(`fast-${elementName}`);
...
describe("A DesignToken", () => {
    it("should support declared types", () => {
        const number = DesignToken.create("number");
        const nil = DesignToken.create("number");
        const bool = DesignToken.create("bool");
        const arr = DesignToken.create("arr");
        const obj = DesignToken.create("obj");
        class Foo {}
        const _class = DesignToken.create("class");
        const sym = DesignToken.create("symbol");
    });
    describe("that is a CSSDesignToken", () => {
        it("should have a createCSS() method that returns a string with the name property formatted as a CSS variable", () => {
            expect(DesignToken.create("implicit").createCSS()).to.equal(
                "var(--implicit)"
            );
        });
        it("should have a readonly cssCustomProperty property that is the name formatted as a CSS custom property", () => {
            expect(DesignToken.create("implicit").cssCustomProperty).to.equal(
                "--implicit"
            );
        });
        it("should have a cssCustomProperty property that aligns with a provided cssCustomPropertyName configuration", () => {
            expect(
                DesignToken.create({
                    name: "name",
                    cssCustomPropertyName: "css-custom-property",
                }).cssCustomProperty
            ).to.equal("--css-custom-property");
        });
    });
    describe("that is not a CSSDesignToken", () => {
        it("should not have a cssCustomProperty property", () => {
            expect(
                "cssCustomProperty" in
                    DesignToken.create({ name: "test", cssCustomPropertyName: null })
            ).to.equal(false);
        });
    ...
    describe("getting and setting a simple value", () => {
        it("should throw if the token value has never been set on the element or it's any ancestors", () => {
            ...
            const token = DesignToken.create("test");
        ...
        it("should support getting and setting falsey values", () => {
            ...
            [false, null, 0, "", NaN].forEach(value => {
                ...
                if (typeof value === "number" && isNaN(value)) {
                    expect(isNaN(token.getValueFor(target))).to.equal(true);
                } else {
        ...
        describe("that is not a CSSDesignToken", () => {
            it("should not set a CSS custom property for the element", () => {
                ...
                token.setValueFor(target, 12);
                expect(
                    window.getComputedStyle(target).getPropertyValue("--test")
                ).to.equal("");
                removeElement(target);
            });
        });
    ...
    describe("getting and setting derived values", () => {
        ...
        it("should get an updated value when other design tokens used in a derived property are changed", () =>
            __awaiter(void 0, void 0, void 0, function* () {
                const target = addElement();
                const tokenA = DesignToken.create("A");
                const tokenB = DesignToken.create("B");
        ...
        describe("that is a CSSDesignToken", () => {
            ...
            it("should update a CSS custom property to the resolved value of a derived token value with a dependent token when the dependent token changes", () =>
                __awaiter(void 0, void 0, void 0, function* () {
                    const target = addElement();
                    const tokenA = DesignToken.create("A");
                    const tokenB = DesignToken.create("B");
            ...
            it("should set a CSS custom property equal to the resolved value for an element in a shadow DOM of a derived token value with a dependent token", () =>
                __awaiter(void 0, void 0, void 0, function* () {
                    const parent = addElement();
    ...
    describe("getting and setting a token value", () => {
        it("should retrieve the value of the token it was set to", () => {
            const tokenA = DesignToken.create("token-a");
            const tokenB = DesignToken.create("token-b");
            const target = addElement();
        ...
        it("should update the derived value of the token when a dependency of the derived value changes", () => {
            const tokenA = DesignToken.create("token-a");
            const tokenB = DesignToken.create("token-b");
            const tokenC = DesignToken.create("token-b");
            const target = addElement();
            tokenA.setValueFor(target, 6);
            tokenB.setValueFor(target, target => tokenA.getValueFor(target) * 2);
        ...
        describe("that is a CSSDesignToken", () => {
            ...
            it("should update the emitted CSS custom property when the token's value changes", () =>
                __awaiter(void 0, void 0, void 0, function* () {
                    const tokenA = DesignToken.create("token-a");
                    const tokenB = DesignToken.create("token-b");
                    const target = addElement();
            ...
            it("should update the emitted CSS custom property of a token assigned a derived value when the token dependency changes", () =>
                __awaiter(void 0, void 0, void 0, function* () {
                    const tokenA = DesignToken.create("token-a");
                    const tokenB = DesignToken.create("token-b");
                    const tokenC = DesignToken.create("token-c");
                    const target = addElement();
                    tokenA.setValueFor(target, 6);
                    tokenB.setValueFor(target, target => tokenA.getValueFor(target) * 2);
                    tokenC.setValueFor(target, tokenB);
    ...
    describe("when used as a CSSDirective", () => {
        it("should set a CSS custom property for the element when the token is set for the element", () => {
            ...
            expect(
                window.getComputedStyle(target).getPropertyValue(token.cssCustomProperty)
            ).to.equal("12");
            removeElement(target);
        });
    ...
    describe("with subscribers", () => {
        ...
        it("should infer DesignToken and CSSDesignToken token types on subscription record", () => {
            DesignToken.create("css").subscribe({
                handleChange(record) {
                    const test = record.token;
                },
            });
            DesignToken.create({ name: "no-css", cssCustomPropertyName: null }).subscribe(
                {
                    handleChange(record) {
                        const test = record.token;
                    },
