 ...
 * @public
 */
function createCarouselTemplate(prefix) {
    return html `
<template
    role="group"
...
>
    <div
        ...
        @keypress=${(x, c) => x.handleRotationKeyDown(c.event)}
    ...
    <div 
        ...
        @click=${(x, c) => x.handleFlipperClick(-1, c.event)}
        @keypress=${(x, c) => x.handleFlipperKeypress(-1, c.event)}
    >
        <slot name="previous-button">
            <${prefix}-flipper
                aria-label="${x => x.previousButtonAriaLabel}" aria-hidden="${x => x.pattern === "tabbed" ? "true" : "false"}"
    ...
    >
        <slot name="next-button">
            <${prefix}-flipper
                aria-label="${x => x.nextButtonAriaLabel}" aria-hidden="${x => x.pattern === "tabbed" ? "true" : "false"}"
                direction=${FlipperDirection.next}
                ${ref("nextFlipperDefault")}
            >
        </slot>
    </div>
    ${when(x => x.pattern === "tabbed", tabbedTemplate)}
    ${when(x => x.pattern === "basic", basicTemplate)}
</template>`;
}
...
class Carousel extends Tabs {
    constructor() {
         ...
         * @internal
         */
        this.handleFlipperKeypress = (direction, e) => {
            switch (e.keyCode) {
                case keyCodeSpace:
                    if (e.target !== this.nextFlipperDefault &&
        ...
        this.stopTime = 0;
        this.incrementSlide = (direction) => {
            const tempLength = this.pattern === "basic" ? this.items.length : this.tabs.length;
            const tempIndex = this.pattern === "basic" ? this.activeSlideIndex : this.activeTabIndex;
            this.focused = false;
            let adjustment = 0;
        ...
        this.handleMouseOver = () => {
            if (!this.paused) {
                ...
                window.clearTimeout(this.pausedTimeout);
                this.stopAutoPlay();
            }
        };
        this.handleMouseLeave = () => {
            if (!this.paused) {
                ...
                const timeDiff = this.autoplayInterval - (this.stopTime - this.startTime);
                if (timeDiff) {
                    window.clearTimeout(this.pausedTimeout);
        ...
        this.handleTabsFocusOut = (e) => {
            ...
            if (!this.contains(e.relatedTarget) &&
                e.relatedTarget !== this.rotationControlDefault) {
                this.firstFocus = true;
            }
        };
        this.handleTabsKeyDown = (e) => {
    ...
    itemsChanged() {
        if (this.items.length && this.pattern === "basic") {
            ...
            if (this.activeSlideId) {
                ...
                this.activeSlideId = this.slideIds[this.activeSlideIndex];
            }
     ...
     * @internal
     */
    handleFlipperClick(direction, e) {
        this.incrementSlide(direction);
    }
    /**
     * @internal
    ...
    adjust(adjustment) {
        ...
        if (this.loop) {
            ...
            this.activeTabIndex = limit(0, this.tabs.length - 1, this.activeTabIndex + adjustment);
        }
        this.setComponent();
    }
    generateSlideIds() {
        this.slideIds = [];
        this.slideIds = this.items.map((item, index) => {
    ...
    startAutoPlay() {
        if (!this.paused) {
            this.stopAutoPlay();
            this.startTime = Date.now();
            this.autoplayTimer = window.setInterval(this.autoplayNextItem, this.autoplayInterval);
        }
    }
    stopAutoPlay() {
    ...
    connectedCallback() {
        ...
        this.rotationControlContainer.addEventListener("mousedown", this.handleRotationMouseDown);
        if (this.pattern === "tabbed") {
            ...
            this.tabPanelsContainerRef.addEventListener("focusout", this.handleMouseLeave);
        }
        else {
...
__decorate([
    attr({ mode: "boolean" })
], Carousel.prototype, "paused", void 0);
__decorate([
    attr({ attribute: "pattern" })
], Carousel.prototype, "pattern", void 0);
__decorate([
    attr({ attribute: "autoplay-interval" })
], Carousel.prototype, "autoplayInterval", void 0);
__decorate([
    attr({ attribute: "active-slide-id" })
], Carousel.prototype, "activeSlideId", void 0);
__decorate([
    attr({ attribute: "aria-labelledby" })
], Carousel.prototype, "arialabelledby", void 0);
__decorate([
    attr({ attribute: "aria-label" })
], Carousel.prototype, "arialabel", void 0);
__decorate([
    attr({ attribute: "next-button-aria-label", mode: "fromView" })
], Carousel.prototype, "nextButtonAriaLabel", void 0);
__decorate([
    attr({ attribute: "previous-button-aria-label", mode: "fromView" })
], Carousel.prototype, "previousButtonAriaLabel", void 0);
__decorate([
    observable
], Carousel.prototype, "notabfocus", void 0);
__decorate([
    observable
], Carousel.prototype, "focused", void 0);
__decorate([
    observable
], Carousel.prototype, "activeSlideIndex", void 0);
__decorate([
    observable
...
function neutralFillInputAlgorithm(indexResolver) {
    return (designSystem) => {
        ...
        return getSwatch(findClosestBackgroundIndex(designSystem) -
            indexResolver(designSystem) * direction, neutralPalette$1(designSystem));
    };
}
/**
 * @internal
...
`.withBehaviors(new DirectionalStyleSheetBehavior(ltr, rtl), neutralForegroundRestBehavior, neutralFocusBehavior);
 ...
 */
let FASTCarousel = class FASTCarousel extends Carousel {
};
FASTCarousel = __decorate([
    customElement({
        name: "fast-carousel",
...
`.withBehaviors(accentForegroundRestBehavior, neutralFillStealthSelectedBehavior, neutralFillStealthActiveBehavior, expandCollapseHoverBehavior, neutralFillStealthHoverBehavior, selectedExpandCollapseHoverBehavior, neutralFillStealthRestBehavior, neutralFocusBehavior, neutralFocusInnerAccentBehavior, neutralForegroundActiveBehavior, neutralForegroundRestBehavior, new DirectionalStyleSheetBehavior(ltr$1, rtl$1), forcedColorsStylesheetBehavior(css `
        :host {
            forced-color-adjust: none;
            border-color: transparent;
            background: ${SystemColors.Field};
