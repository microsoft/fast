...
export function accessibleAlgorithm(
    ...
    return (designSystem: FASTDesignSystem): SwatchFamily => {
        const resolvedPalette: Palette = evaluateDesignSystemResolver(
            palette,
            designSystem
        );
        ...
        const resolvedRest: number = evaluateDesignSystemResolver(
            restDelta,
            designSystem
        );
        const resolvedHover: number = evaluateDesignSystemResolver(
            hoverDelta,
            designSystem
        );
        const resolvedActive: number = evaluateDesignSystemResolver(
            activeDelta,
            designSystem
        );
        const resolvedFocus: number = evaluateDesignSystemResolver(
            focusDelta,
            designSystem
        );
