...
import { ITextareaAttributeSettingsMappingToPropertyNames } from "./form.props";
import {
...
class FormSection extends React.Component<IFormSectionProps, IFormSectionState> {
    ...
    public componentWillUpdate(nextProps: IFormSectionProps, nextState: IFormSectionState): void {
        ...
        if (nextProps.schema !== this.props.schema) {
            if (nextProps.schema.oneOf || nextProps.schema.anyOf) {
                state.oneOfAnyOf = {};
                state.oneOfAnyOf.type = nextProps.schema.oneOf ? "oneOf" : "anyOf";
                state.oneOfAnyOf.activeIndex = getOneOfAnyOfActiveIndex(
                    state.oneOfAnyOf.type,
                    nextProps.schema,
                    nextProps.data
                );
    ...
    private generateFormElement(property: any, index: number, location: string, required: boolean, label: string): JSX.Element | any {
        let dataLocation: string = isRootLocation(this.props.dataLocation) || isRootLocation(location)
        ...
        const formItemProps: IFormItemCommon = {
            ...
            index,
            dataLocation,
            data: isRootLocation(location) ? this.props.data : get(this.props.data, location),
            required,
            label: label === "" && this.state.schema.title !== void(0) ? this.state.schema.title : label,
            onChange: this.props.onChange
        };

        if (this.props.componentMappingToPropertyNames) {
        ...
        if (typeof property.enum !== "undefined" && property.enum.length > 0) {
    ...
    private getCategories(formItemParameters: IFormItemParameters[]): JSX.Element[] {
        ...
        let categoryParams: IFormCategories[] = [];

        for (const formItemParameter of formItemParameters) {
            let assignedCategory: string = "Default";
            let assignedCategoryWeight: number = this.props.orderByPropertyNames.defaultCategoryWeight || 0;
            let assignedItemWeight: number = 0;
            ...
            for (const category of this.props.orderByPropertyNames.categories) {
                for (const categoryProperty of category.properties) {
                    if (Array.isArray(categoryProperty.propertyName)) {
                        for (const propertyName of categoryProperty.propertyName) {
                            if (propertyName === formItemParameter.item) {
                                assignedCategory = category.title;
                                assignedCategoryWeight = category.weight;
                                assignedItemWeight = categoryProperty.weight || assignedItemWeight;
                            }
                        }
                    } else if (categoryProperty.propertyName === formItemParameter.item) {
                        assignedCategory = category.title;
                        assignedCategoryWeight = category.weight;
                        assignedItemWeight = categoryProperty.weight || assignedItemWeight;
                    }
            ...
            for (let i: number = 0, categoryParamsLength: number = categoryParams.length; i < categoryParamsLength; i++) {
                if (assignedCategory === categoryParams[i].title) {
                    categoryExists = true;
                    categoryIndex = i;
                }
            }
            ...
            if (categoryExists) {
                categoryParams[categoryIndex].items.push({
                    weight: assignedItemWeight,
                    params: formItemParameter
                });
            } else {
                categoryParams.push({
                    title: assignedCategory,
                    weight: assignedCategoryWeight,
                    items: [{
                        weight: assignedItemWeight,
                        params: formItemParameter
                    }]
        ...
        categoryParams = this.getWeightedCategoriesAndItems(categoryParams);
    ...
    private generateFormObject(property: any, location: string, required: string[], not?: string[]): JSX.Element[] {
        ...
        const formItemParameters: IFormItemParameters[] = [];
        ...
        if (property.properties) {
            ...
            Object.keys(property.properties).forEach((item: any, index: number) => {
                ...
                if (!isNotRequired) {
                    ...
                    formItems.push(this.generateFormElement(params.property, params.index, params.item, params.isRequired, params.title));

                    if (params.property.type !== "object" && typeof params.property.properties === "undefined") {
                        formItemParameters.push(params);
                    }
                }
            });
            ...
            if (this.props.orderByPropertyNames) {
                if (
                    typeof this.props.orderByPropertyNames.showCategoriesAtPropertyCount === "number" &&
                    this.props.orderByPropertyNames.showCategoriesAtPropertyCount >= formItems.length
                ) {
                    return formItems;
                }

                return this.getCategories(formItemParameters);
            }
        }
