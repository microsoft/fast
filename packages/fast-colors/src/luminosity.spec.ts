...
describe("luminance", (): void => {
    test("should adjust a color to an approximate luminance value", (): void => {
        const targetLuminance: number = 0.5;

        expect(
            parseFloat(
                Chroma(luminance(targetLuminance, Chroma("#FFF")))
                    .luminance()
                    .toPrecision(2)
            )
        ).toBe(targetLuminance);
        expect(
            parseFloat(
                Chroma(luminance(targetLuminance, Chroma("#000")))
                    .luminance()
                    .toPrecision(2)
            )
        ).toBe(targetLuminance);
        expect(
            parseFloat(
                Chroma(luminance(targetLuminance, Chroma("#067983")))
                    .luminance()
                    .toPrecision(2)
            )
        ).toBe(targetLuminance);
        expect(
            parseFloat(
                Chroma(luminance(targetLuminance, Chroma("red")))
                    .luminance()
                    .toPrecision(2)
            )
        ).toBe(targetLuminance);
        expect(
            parseFloat(
                Chroma(luminance(targetLuminance, Chroma("green")))
                    .luminance()
                    .toPrecision(2)
            )
        ).toBe(targetLuminance);
        expect(
            parseFloat(
                Chroma(luminance(targetLuminance, Chroma("blue")))
                    .luminance()
                    .toPrecision(2)
            )
        ).toBe(targetLuminance);
...
describe("luminanceSwitch", (): void => {
    ...
    test("should return a function that returns the second argument when the foreground luminance is equal to background luminance and both are greater than .5", (): void => {
        expect(luminanceSwitch(0.51, 0.51)(true, false)).toBe(false);
        expect(luminanceSwitch(0.51, 0.51)(true, false)).toBe(false);
    });
    /* tslint:disable-next-line */
    test("should return a function that returns the second argument when the foreground luminance is equal to background luminance and both are .5 or less", (): void => {
        expect(luminanceSwitch(0.5, 0.5)(true, false)).toBe(true);
