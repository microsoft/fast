...
import { quantize } from "./color-quantization";
import { extractPalette } from "./palette-extractor";
describe("Extracting a palette from a set of colors", () => {
    test("Extract palette with default settings", () => {
        ...
        const results = extractPalette(colors);
    ...
    test("Extract palette with default settings from greyscale image", () => {
        const pixels = new ArrayPixelBlob(testGrey200ImageData, 200, 125);
        const colors = quantize(pixels);
        const results = extractPalette(colors);
        expect(results.length).toBe(6);
