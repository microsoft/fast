...
describe("Palette generation", () => {
    ...
    test("generateScaledPalettes", () => {
        const results1: {
            short: ColorRGBA64[];
            long: ColorRGBA64[];
        } | null = generateScaledPalettes(parseColorHexRGB("#d9b6f3"));

        expect(results1).not.to.be.null;
        ...
        const results2: {
            short: ColorRGBA64[];
            long: ColorRGBA64[];
        } | null = generateScaledPalettes(parseColorHexRGB("#006699"), 11, {
            targetSize: 63,
            spacing: 5,
        ...
        expect(results2).not.to.be.null;
    ...
    test("rescale", () => {
        const testPalette1: ColorPalette | null = generateOffCenterPalette(
            parseColorHexRGB("#320198"),
            11
        );
        expect(testPalette1).not.to.be.null;
