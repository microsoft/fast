...
describe("accentFill", (): void => {
    const neutralPalette = PaletteRGB.create(SwatchRGB.create(.5, .5, .5));
    ...
    it("should have accessible rest and hover colors against accentForegroundCut", (): void => {
        ...
        accentColors.forEach((accent: SwatchRGB): void => {
            ...
            neutralPalette.swatches.forEach((swatch: SwatchRGB): void => {
                ...
                expect(
                    accentForegroundCutColor.contrast(accentFillColors.rest)
                ).to.be.gte(4.5);
                expect(
                    accentForegroundCutColor.contrast(accentFillColors.hover)
                ).to.be.gte(4.5);
                expect(
                    accentForegroundCutColor.contrast(accentFillLargeColors.rest)
                ).to.be.gte(3);
                expect(
                    accentForegroundCutColor.contrast(accentFillLargeColors.hover)
                ).to.be.gte(3);
            });
        });
    });
