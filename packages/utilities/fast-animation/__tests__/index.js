...
describe("getPropertiesToAnimate", () => {
    ...
    test('should return an array with both "transform" and "opacity" when opacity and x are set', () => {
        const properties = new AnimateMock(fakeElement, {
            x: 20,
            opacity: 0,
...
describe("getOptionKeyframeValues", () => {
    ...
    test("should correclty create opacity keyframe value", () => {
        expect(new AnimateMock(fakeElement, { opacity: 0 })["getOptionKeyframeValues"]()).toEqual({
            opacity: "0",
        });
    });
    test("should correclty create x keyframe value", () => {
        expect(new AnimateMock(fakeElement, { x: 20 })["getOptionKeyframeValues"]()).toEqual({
            transform: "translateX(20px)",
        });
    });
    test("should correclty create opacity keyframe value", () => {
        expect(new AnimateMock(fakeElement, { y: 20 })["getOptionKeyframeValues"]()).toEqual({
    ...
    test("should correclty create opacity keyframe value", () => {
        expect(new AnimateMock(fakeElement, { scale: 20 })["getOptionKeyframeValues"]()).toEqual({
            transform: "scale(20)",
        });
    });
    test("should correclty create opacity keyframe value", () => {
        expect(new AnimateMock(fakeElement, { scale: [20, 10] })["getOptionKeyframeValues"]()).toEqual({
            transform: "scale(20,10)",
        });
    });
    test("should correclty create opacity keyframe value", () => {
        expect(new AnimateMock(fakeElement, { rotate: 20 })["getOptionKeyframeValues"]()).toEqual({
...
describe("getOptionKeyframes", () => {
    ...
    test("should correctly return keyframes with the scale option", () => {
        expect(new AnimateMock(fakeElement, { scale: 20 })["getOptionKeyframes"]()).toEqual([{ transform: "" }, { transform: "scale(20)" }]);
    });
    test("should correctly return keyframes with the independent scale option", () => {
        expect(new AnimateMock(fakeElement, { scale: [20, 10] })["getOptionKeyframes"]()).toEqual([{ transform: "" }, { transform: "scale(20,10)" }]);
    });
    test("should correctly return keyframes with the rotate option", () => {
        expect(new AnimateMock(fakeElement, { rotate: 20 })["getOptionKeyframes"]()).toEqual([{ transform: "" }, { transform: "rotate(20deg)" }]);
    });
    ...
    test("should correctly return keyframes with both two transform functions", () => {
        expect(new AnimateMock(fakeElement, { x: 20, scale: 2 })["getOptionKeyframes"]()).toEqual([{ transform: "" }, { transform: "translateX(20px) scale(2)" }]);
    });
});
