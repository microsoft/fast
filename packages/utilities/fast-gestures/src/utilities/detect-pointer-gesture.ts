...
export function detectPointerGesture(
  ...
  const directionsLength: number = translationDirection === PointerTranslationDirection.Cardinal ? 4 : 8;
  const vectorAngle: number = getVectorAngle(x, y);
  const directionIndex: number = Math.floor((vectorAngle / (2 * Math.PI)) * directionsLength);
