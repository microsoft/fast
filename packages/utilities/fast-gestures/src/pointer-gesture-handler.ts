...
export class PointerGestureHandler extends Disposable implements IDisposable {
  ...
  constructor(_target: Window | HTMLElement | null = window, options?: IPointerGestureOptions) {
    ...
    this.target = _target;
    this.options = options !== undefined ? { ...DEFAULT_OPTIONS, ...options } : DEFAULT_OPTIONS;
    this.target?.addEventListener(PointerEventType.Down, this.handlePointerStart, { passive: false });
    this.target?.addEventListener(PointerEventType.Move, this.handlePointerMove, { passive: false });
    this.target?.addEventListener(PointerEventType.Up, this.handlePointerEnd, { passive: false });
    this.target?.addEventListener(PointerEventType.Cancel, this.handlePointerEnd, { passive: false });

    if (this.options.isHoverEnabled) {
      this.target?.addEventListener(PointerEventType.Enter, this.handlePointerEnter, { passive: false });
      this.target?.addEventListener(PointerEventType.Leave, this.handlePointerLeave, { passive: false });
    }
  }
  ...
  private addPointerInfo(e: PointerEvent): IPointerInfoInternal {
    ...
    if (
      ...
      const initialRotationAngle: number = Math.floor(
        (getRotationAngle(pointer1.x, pointer1.y, pointer2.x, pointer2.y) * 100) / 100
      );
  ...
  private updateMultiPointerGesture(multiPointer: IPointerInfoInternal): IPointerInfoInternal {
    ...
    if (this.shouldDetectGesture[PointerGestureFeature.Rotate]) {
      ...
      const isRotationClockwise: boolean =
        rotationAngle > multiPointer._previousRotationAngle + rotationAngleThreshold!;
      const isRotationCounterClockwise: boolean =
        rotationAngle + rotationAngleThreshold! < multiPointer._previousRotationAngle;

      multiPointer.rotationAngle = Math.floor(rotationAngle * 100) / 100;

      if (Math.abs(multiPointer._previousRotationAngle - multiPointer.rotationAngle) > 180 / Math.PI) {
      ...
      if (isRotationClockwise || isRotationCounterClockwise) {
        if (isRotationClockwise) {
          ...
          multiPointer.rotationDirection = PointerRotationDirection.CounterClockwise;
    ...
    if (this.shouldDetectGesture[PointerGestureFeature.Pinch]) {
      ...
      const isPinchOut: boolean = pointerDistance > multiPointer._previousPinchDistance + pinchDistanceThreshold!;
      const isPinchIn: boolean = pointerDistance < multiPointer._previousPinchDistance - pinchDistanceThreshold!;
  ...
  private handlePointerMove = (e: PointerEvent): void => {
    const isDrag: boolean =
      (e.pointerType === PointerInputType.Mouse && this.isDown(e)) || e.pointerType === PointerInputType.Touch;
    ...
    if (
      ...
      (e.pointerType === PointerInputType.Pen && this.isDown(e) && !this.options.isHoverEnabled) // Pen move
    ) {
      this.activePointer = this.updatePointerInfo(e);
  ...
  private handlePointerEnd = (e: PointerEvent): void => {
    ...
    if (this._pointers.size > 1) {
      ...
      const isValidSwipe: boolean = this.shouldDetectGesture[PointerGestureFeature.Swipe]
        ? ((Math.abs(this.activePointer.xDelta) > this.options.swipeThresholdX! &&
            Math.abs(this.activePointer.xVelocity) > this.options.swipeVelocityThreshold!) ||
            (Math.abs(this.activePointer.yDelta) > this.options.swipeThresholdY! &&
              Math.abs(this.activePointer.yVelocity) > this.options.swipeVelocityThreshold!)) &&
          e.buttons === 0
        : false;

      // If Swipe is valid then get the Swipe Direction from previous Pan direction
      if (isValidSwipe) {
        this.activePointer.gestureType = getSwipeDirection(this.previousPanDirection);
      ...
      else if (this.shouldDetectGesture[PointerGestureFeature.DoubleTap] && this.doubleTapIsWaiting) {
      ...
      // Otherwise
        ...
        if (this.shouldDetectGesture[PointerGestureFeature.LongPress] && this.activePointer.isLongPressComplete) {
        ...
        // If a single Tap has been detected
          ...
          this.activePointer.pointerType === PointerInputType.Touch &&
          this.activePointer.lastUpdateTime - this.activePointer.initialPressTime < this.options.singleTapTime!
        ) {
  ...
  private handleLongPress = (raf: number): void => {
    ...
    const currentLongPressTime: number = Math.max(0, raf - this.activePointer.initialPressTime);

    this.activePointer.longPressProgress =
      Math.round((currentLongPressTime / this.options.longPressDuration!) * 100) / 100;
  ...
  public dispose(): void {
    ...
    if (this.options.isHoverEnabled) {
      this.target?.removeEventListener(PointerEventType.Enter, this.handlePointerEnter, false);
      this.target?.removeEventListener(PointerEventType.Leave, this.handlePointerLeave, false);
    }

    this.target?.removeEventListener(PointerEventType.Down, this.handlePointerStart, false);
    this.target?.removeEventListener(PointerEventType.Move, this.handlePointerMove, false);
    this.target?.removeEventListener(PointerEventType.Up, this.handlePointerEnd, false);
    this.target?.removeEventListener(PointerEventType.Cancel, this.handlePointerEnd, false);
  ...
  public onGesture(gesture: PointerGesture, listener: (args: IPointerGestureInfo) => any): IDisposable {
    if (gesture === PointerGesture.General) {
      ...
      Object.keys(this.shouldDetectGesture).every(
        (gestureType: PointerGestureFeature) => (this.shouldDetectGesture[gestureType] = true)
      );
    } else {
