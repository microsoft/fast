...
describe("parseQueryStringParams", () => {
    it("basic query string", () => {
        const params = parseQueryStringParams("www.microsoft.com?a=12345&b=qwerty");
        expect(params.size).to.equal(2);
        expect(params.get("a")).to.equal("12345");
        expect(params.get("b")).to.equal("qwerty");
    });
    it("query string not part of a full url", () => {
        const params = parseQueryStringParams("a=12345&b=qwerty");
        expect(params.size).to.equal(2);
        expect(params.get("a")).to.equal("12345");
        expect(params.get("b")).to.equal("qwerty");
    });
    it("query string with encoding", () => {
    ...
    it("undefined or empty input should return an empty Map", () => {
        const params1 = parseQueryStringParams("");
        expect(params1.size).to.equal(0);
        const params2 = parseQueryStringParams(undefined);
        expect(params2.size).to.equal(0);
    });
    it("gibberish input should return an empty Map", () => {
        const params = parseQueryStringParams("qwertyuiopasdfghjklzxcvbnm");
