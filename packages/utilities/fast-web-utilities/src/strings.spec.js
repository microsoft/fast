...
describe("format", () => {
    it("should correctly manage undefined by returning an unformatted string", () => {
        const formatterString = "Hello {0} world";
        expect(format(formatterString, undefined)).to.equal("Hello  world");
    });
    it("should correctly manage null by returning an unformatted string", () => {
        const formatterString = "Hello {0} world";
        expect(format(formatterString, null)).to.equal("Hello  world");
    });
    it("should correctly manage having too many parameters", () => {
        const formatterString = "View {0} {1}";
        expect(format(formatterString, "page", "five", "now")).to.equal("View page five");
    });
    it("should correctly manage a formatter with not enough parameters", () => {
        const formatterString = "View {0} {1}";
        expect(format(formatterString, "page")).to.equal("View page {1}");
    });
    it("should correctly manage empty strings by returning a formatted string with white space", () => {
        const formatterString = "Hello {0} world";
        expect(format(formatterString, "")).to.equal("Hello  world");
    });
    it("should correctly manage strings by returning a formatted string", () => {
        const formatterString = "Hello {0} world";
        expect(format(formatterString, "foo")).to.equal("Hello foo world");
    });
    it("should correctly manage multiple strings parameters", () => {
        const formatterString = "View {0} {1}";
        expect(format(formatterString, "page", "five")).to.equal("View page five");
    });
    it("should correctly manage non-formatted strings by returning the initial string", () => {
        const formatterString = "Hello";
...
describe("pascalCase", () => {
    it("should correctly manage hyphenated strings", () => {
        expect(pascalCase("string-extensions")).to.equal("StringExtensions");
    });
    it("should correctly manage strings with whitespace", () => {
        expect(pascalCase(" foo bar ")).to.equal("FooBar");
    });
    it("should correctly manage all caps strings", () => {
        expect(pascalCase("STRING EXTENSIONS")).to.equal("StringExtensions");
    });
});
describe("spinalCase", () => {
