...
module.exports = function(schema, propName) {
    ...
    if (schema.definitions) {
        propertyLocations = propertyLocations.concat(
            findPropsInDefinitions(schema, "definitions", propName)
        );
    }
    ...
    if (keys && keys.length > 0) {
        for (let key of keys) {
            matches = matches.concat(
                findProps(get(definitions, key), `${location}.${key}`, propName)
            );
        }
    }
...
let findProps = function(schema, location, propName) {
            ...
            if (schema.properties[key].items) {
                let newLocation =
                    location === ""
                        ...
                        : `${location}.properties.${key}.items`;

                if (!isRecursive(newLocation, repeatingPattern)) {
                    matches = matches.concat(
                        findProps(schema.properties[key].items, newLocation, propName)
                    );
                }
            } else {
                let newLocation =
                ...
                if (!isRecursive(newLocation, repeatingPattern)) {
                    matches = matches.concat(
                        findProps(schema.properties[key], newLocation, propName)
                    );
                }
            }
        }
    ...
    if (schema.allOf || schema.oneOf || schema.anyOf) {
        ...
        for (
            let i = 0, schemaArrayLength = schema[oneOfAnyOfAllOf].length;
            i < schemaArrayLength;
            i++
        ) {
            ...
            if (arrayItem[propName]) {
                ...
                for (let key of keys) {
                    let newLocation =
                        location === ""
                            ? `${oneOfAnyOfAllOf}[${i}].properties.${key}`
                            : `${location}.${oneOfAnyOfAllOf}[${i}].properties.${key}`;

                    if (!isRecursive(newLocation, repeatingPattern)) {
                        matches = matches.concat(
                            findProps(arrayItem.properties[key], newLocation, propName)
                        );
                    }
                }
                ...
                if (!isRecursive(newLocation, repeatingPattern)) {
                    matches = matches.concat(
                        findProps(arrayItem.items, newLocation, propName)
                    );
                }
            }
