...
const resolveOneOfAnyOf = function(schemaArray) {
    ...
    for (let schema of schemaArray) {
        ...
        const oneOfAnyOfSchemaLocations = oneOfSchemaLocations.concat(anyOfSchemaLocations);
        locationTotals += oneOfAnyOfSchemaLocations.length;

        if (oneOfAnyOfSchemaLocations.length > 0) {
            let oneOfAnyOfItems;
            let oneOfAnyOf = (get(schema, `${oneOfAnyOfSchemaLocations[0]}.oneOf`) ||
                              get(schema, `${oneOfAnyOfSchemaLocations[0]}oneOf`))
                              ? 'oneOf' : 'anyOf';

            oneOfAnyOfItems = (oneOfAnyOfSchemaLocations[0] === '') 
