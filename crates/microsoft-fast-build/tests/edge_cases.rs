...
use microsoft_fast_build::{render, render_with_locator, render_entry_with_locator, JsonValue, RenderConfig};
...
fn test_deeply_nested_missing_intermediate_returns_error() {
    ...
    let e = render_template("{{a.b.c.d}}", r#"{"a": {"b": {}}}"#, &RenderConfig::default())
...
fn test_nested_custom_element_inside_shadow() {
    ...
    let result = render_with_locator("<my-outer></my-outer>", &empty_root(), &locator, &RenderConfig::default())
...
fn test_render_with_json_value_directly() {
    ...
    let result = render("<p>{{greeting}}</p>", &state, &RenderConfig::default()).expect("render with JsonValue directly");
