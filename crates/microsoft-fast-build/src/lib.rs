...
pub fn render_template(template: &str, state: &str, config: &RenderConfig) -> Result<String, RenderError> {
...
pub fn render(template: &str, state: &JsonValue, config: &RenderConfig) -> Result<String, RenderError> {
...
pub fn render_with_locator(template: &str, state: &JsonValue, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
...
pub fn render_template_with_locator(template: &str, state: &str, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
    let state_value = json::parse(state).map_err(|e| RenderError::JsonParse { message: e.message })?;
...
pub fn render_entry_with_locator(template: &str, state: &JsonValue, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
...
/// Custom elements found at this level receive the full root state rather than
/// building their child state from HTML attributes.
pub fn render_entry_template_with_locator(template: &str, state: &str, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
    let state_value = json::parse(state).map_err(|e| RenderError::JsonParse { message: e.message })?;
