...
/// Render a FAST HTML template with a parsed [`JsonValue`], a [`Locator`], and a [`RenderConfig`].
pub fn render_with_locator_and_config(template: &str, state: &JsonValue, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
    renderer::render_with_locator_and_config(template, state, locator, config)
}

/// Render a FAST HTML template with a JSON state string, a [`Locator`], and a [`RenderConfig`].
pub fn render_template_with_locator_and_config(template: &str, state: &str, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
    let state_value = json::parse(state).map_err(|e| RenderError::JsonParse { message: e.message })?;
    renderer::render_with_locator_and_config(template, &state_value, locator, config)
}

/// Render the top-level **entry HTML** with a parsed [`JsonValue`], a [`Locator`], and a [`RenderConfig`].
/// Custom elements found at this level receive the full root state rather than
/// building their child state from HTML attributes.
pub fn render_entry_with_locator_and_config(template: &str, state: &JsonValue, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
    renderer::render_entry_with_locator_and_config(template, state, locator, config)
}

/// Render the top-level **entry HTML** with a JSON state string, a [`Locator`], and a [`RenderConfig`].
/// Custom elements found at this level receive the full root state rather than
/// building their child state from HTML attributes.
pub fn render_entry_template_with_locator_and_config(template: &str, state: &str, locator: &Locator, config: &RenderConfig) -> Result<String, RenderError> {
    let state_value = json::parse(state).map_err(|e| RenderError::JsonParse { message: e.message })?;
    renderer::render_entry_with_locator_and_config(template, &state_value, locator, config)
}
