...
ARIA attributes follow the [Element ARIA reflection](https://developer.mozilla.org/en-US/docs/Web/API/Element#aria) convention: camelCase property names (e.g. `ariaDisabled`) correspond to kebab-case `aria-*` HTML attributes (e.g. `aria-disabled`).

#### Passing `aria-*` attributes to custom elements

When a custom element receives `aria-*` attributes, the renderer converts them to their camelCase property name and stores them as top-level state keys:
...
The `aria-*` → `ariaX` mapping uses the same camelCase conversion as the browser: `aria-disabled` → `ariaDisabled`, `aria-value-now` → `ariaValueNow`.
...
**`aria-*` attributes** are converted to their camelCase ARIA property name (e.g. `aria-disabled` → `ariaDisabled`) and stored as top-level state keys, following the [Element ARIA reflection](https://developer.mozilla.org/en-US/docs/Web/API/Element#aria) convention.
