...
- `examples/ssr/webui-todo-app/` — A To-Do app demonstrating FAST declarative HTML with `@microsoft/webui` prerendering and `@microsoft/fast-element`'s `declarativeTemplate()` + `enableHydration()` for client-side hydration. Light theme.
