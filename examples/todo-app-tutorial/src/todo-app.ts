import { customElement, FASTElement, html, css, attr, repeat, observable, ExecutionContext } from "@microsoft/fast-element";
import { typography } from './typography';
import { TodoItem } from './todo-item';

function detail<T = any>(ctx: ExecutionContext) {
...
const template = html<TodoApp>`
  ...
  <todo-form @todo-submit=${(x, c) => x.addTodo(detail(c))}></todo-form>

  <ul>
    ${repeat(x => x.todos, html<TodoItem, TodoApp>`
      <li>
        <fast-checkbox checked=${x => x.done} @change=${x => x.toggleDone()}></fast-checkbox>
        <span class="list-text ${x => x.done ? 'done' : ''}">
          ${x => x.description}
        </span>
        <fast-button @click=${(x, c) => c.parent.removeTodo(x)}>&times;</fast-button>
      </li>
    `)}
  </ul>
`;
