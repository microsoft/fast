...
import { FASTDesignToken } from "./fast-design-token.js";
...
const neutralPalette = new FASTDesignToken("neutral-palette");
const fillColor = new FASTDesignToken("fill-color");
@customElement({
    name: "my-root",
    styles: css``,
    template: html`
...
export class MyRoot extends FASTElement {
    ...
    connectedCallback(): void {
        ...
        fillColor.setValueFor(root, (resolve: any) => {
            return resolve(neutralPalette)[0];
        });
    }
...
export class MyCard extends FASTElement {
    connectedCallback(): void {
        super.connectedCallback();
        fillColor.setValueFor(this, (resolve: any) => {
            const palette = resolve(neutralPalette);
