...
`examples/chat-app` is a small end-to-end FAST example that combines three ideas:
...
This example follows the same top-level shape as `examples/todo-app/` and adds the extra declarative-rendering source files needed for FAST build:
...
- `entry.html` — source HTML passed to `@microsoft/fast-build`.
...
- `src/main.ts` — enables hydration and defines the custom elements.
...
`src/main.ts` calls `enableHydration()` first, then defines:
