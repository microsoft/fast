...
Make sure to document the migration strategy in a migration document in the package(s) that has breaking changes, eg. `packages/fast-element/docs/migration/fast-element-3.md`.

Example of how to format a migration document:
...
Releases are produced from a dedicated **bump pull request** authored by a maintainer (not by CI). Once the bump PR lands on `main`, two Azure Pipelines under [`.ado/pipelines/`](.ado/pipelines/) own the rest: `azure-pipelines-build.yml` (**`FAST - CD Build`**) packs the freshly-bumped npm tarballs and paired Rust crates for every package whose release tag doesn't exist yet, and `azure-pipelines-cd.yml` (**`FAST - CD`**) validates those artifacts, publishes to npm and crates.io, and then creates the GitHub release and git tag per package. The detailed CD design is documented in [`.github/workflows/README.md`](.github/workflows/README.md).
...
After merge, the [`FAST - CD Build`](.ado/pipelines/azure-pipelines-build.yml) pipeline triggers automatically on the push to `main`. Its `PrepareRelease` stage selects the new `${name}_v${version}` tags that don't yet exist on `origin` and hands that exact list to `BuildArtifacts`. Before packing, `BuildArtifacts` re-checks every selected tag and fails the batch if a concurrent release created any of them; otherwise it packs exactly the selected packages' `.tgz` files (and any paired `.crate` files) into pipeline artifacts. Completion of that stage on `main` triggers [`FAST - CD`](.ado/pipelines/azure-pipelines-cd.yml), which validates the artifacts, reserves each pending package/version with its release tag, and publishes the whole npm/crate batch through one `FAST.Release.PipelineTemplate` invocation. Only after that combined publication succeeds does it create `deployed/${name}_v${version}` markers at the validated release commit, followed by the GitHub Releases. Release tags prevent concurrent publication attempts; deployment markers provide a separate audit trail of successful registry publication. See [`.github/workflows/README.md`](.github/workflows/README.md) for the full failure and retry behavior.
